...
POLICY_PATH = os.path.join(robomimic.__path__[0], "../diffusion_policy_trained_models/maze2d/20240620205734/models/model_epoch_1200.pth") # policy checkpoint
...
def rollout(policy, env, horizon, **render_kwargs):
    ...
    video_path = os.path.join(result_dir, f"safe_rollout_{env.name}_{render_mode}.mp4")
...
if __name__ == "__main__":
    ...
    for rand_seed in rand_seeds:
        ...
        # Classic `robomimic-style` loading policy and env
        ...
        policy     = SafeDiffusionPolicy(rollout_policy = policy, backup_policy = Simple2DPlanner(), **config.safety)

        # Wrap the environment with the zonotope world
