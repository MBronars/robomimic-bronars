...
CONFIG_PATH = os.path.join(os.path.dirname(__file__),
                           "../exps/batchdiffusion/safediffusion_arm.json")
...
def test(policy, env, horizon, render_mode, seed, save_dir, camera_names,
    ...
    for step_i in range(horizon):
        ...
        act, controller_to_use = policy(ob = obs, num_samples = 10)
        ...
        # Offline rendering
            ...
            if step_i % video_skip == 0:
                if render_mode == "rgb_array":
                    ...
                    plans = [v["plan"] for v in policy.ranking_info.values()]
                    eef_plans = [policy.backup_policy.get_grasping_pos_from_plan(plan = plan, T_world_to_base = obs["T_world_to_base"])
                                 for plan in plans]
                    eef_plan = policy.backup_policy.get_grasping_pos_from_plan(
...
if __name__ == "__main__":
    # ----------------------------------------- #
    # Test-seed: [11, 35, 74]
    # NOTE: TODO inspect seed 13, optimization
