...
CONFIG_PATH = os.path.join(os.path.dirname(__file__),
                           "../exps/backup_planner_qpos/safediffusion_arm.json")

def test(policy, env, horizon, render_mode, seed, save_dir, camera_names, target_qpos,
    ...
    goal = env.set_goal(qpos = target_qpos)
...
if __name__ == "__main__":
    ...
    for seed in seeds:
        target_qpos      = np.random.uniform(-np.pi/2, np.pi/2, 7)
        stats = test(
