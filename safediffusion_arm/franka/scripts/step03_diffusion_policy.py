        ...
        # Offline rendering
            ...
            if step_i % video_skip == 0:
                if render_mode == "rgb_array":
                    ...
                    # FO_arm  = policy.backup_policy.get_arm_zonotopes_at_q(q = policy.backup_policy.to_tensor(obs["robot0_joint_pos"]))
                    
                    # T_world_to_arm_base = policy.backup_policy.to_tensor(obs["T_world_to_arm_base"])
                    # T_arm_base_to_gripper_base = policy.backup_policy.get_arm_link_pose_at_q("right_hand", obs["robot0_joint_pos"]) 
                    # T_world_to_gripper_base = T_world_to_arm_base @ T_arm_base_to_gripper_base

                    # FO_gripper = policy.backup_policy.get_gripper_zonotopes_at_q(q = policy.backup_policy.to_tensor(obs["robot0_gripper_qpos"]),
                    ...
                    FRS = policy.backup_policy.get_forward_occupancy_from_plan(plan = policy.nominal_plan, 
                                                                               vis_gripper = True, 
                                                                               gripper_init_qpos = obs["robot0_gripper_qpos"])
