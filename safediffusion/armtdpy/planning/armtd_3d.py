...
class ARMTD_3D_planner():
    def __init__(self,env,dt_plan=0.02,zono_order=40,max_combs=200,dtype=torch.float,device=torch.device('cpu')):
    ...
    def prepare_constraints(self,qpos,qvel,obstacles):
        """ Prepare the constraints for the optimization problem. Given a state (qpos, qvel), it first computes 
        the set of all possible forward occupancy of the links with varying joint acceleration parameter with
        fixed qpos and initial velocity qvel. Then, it constructs the polytopic constraints by turning into the
        intersecting problem of the forward occupancy and the obstacles. It also sets current qpos and qvel 
        for the planner, which is used for the optimization problem.

        Args:
            qpos: The initial joint position
            qvel: The initial joint velocity
            obstacles: The list of obstacles in the environment
    ...
    def plan(self,env,ka_0,time_limit=5.0):
        """ Compute the optimization problem. Given the environment, it first prepares the constraints
        using current (qpos, qvel) of the robot. Then, it solves the optimization problem to find the
        optimal joint acceleration parameter ka that minimizes the objective function, which is the 
        squared error between the planned trajectory and the goal trajectory.

        Args:
            env: The environment that has goal, obstacle, and robot's current state (qpos, qvel).
            ka_0: The initial guess of joint acceleration parameter, used for the optimization problem
            time_limit: The time limit for the optimization problem
        
        Output:
            k_opt: The optimal joint acceleration parameter that minimizes the objective function
            flag: The status of the optimization problem (0: optimal, 1: infeasible, 2: failed, 3: error)
            time: The time taken to solve the optimization problem
        """
        t0 = time.time()
    ...
    def rollout_param(self, q0, kv, ka):
        ...
        # First piece trajectory (acclerate)
