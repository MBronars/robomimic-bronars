...
if use_zonopy:
    from zonopy.contset import matPolyZonotope, batchMatPolyZonotope
else:
    from safediffusion.armtdpy.reachability.conSet import matPolyZonotope, batchMatPolyZonotope
...
def gen_batch_rotatotope_from_jrs_trig(bPZ,rot_axis):
    """
    This is not rotatope, rather, this is matrix zonotope (ARMTD, Eq (12))
    """
    ...
    cosq = bPZ.c[bPZ.batch_idx_all+(slice(cos_dim,cos_dim+1),)].unsqueeze(-1) # [BS, 1, 1]
    sinq = bPZ.c[bPZ.batch_idx_all+(slice(sin_dim,sin_dim+1),)].unsqueeze(-1) # [BS, 1, 1]
    C = torch.eye(3,dtype=dtype,device=device) + sinq*w_hat + (1-cosq)*w_hat@w_hat # Rotation Matrix [BS, 3, 3]
    cosq = bPZ.Z[bPZ.batch_idx_all+(slice(1,None),slice(cos_dim,cos_dim+1))].unsqueeze(-1) # cos-dim of generator matrix
    sinq = bPZ.Z[bPZ.batch_idx_all+(slice(1,None),slice(sin_dim,sin_dim+1))].unsqueeze(-1) # sin-dim of generator matrix
