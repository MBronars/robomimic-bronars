"""
Utility functions useful for the robot kinematics
...
class KinovaGen3IK():
    def __init__(self):
        ...
        self.world.loadElement(parent_path+"/assets/ik/kinova_with_robotiq_85.rob")
        # self.world.loadElement(parent_path+"/robots/kinova_with_robotiq_85.urdf")
        # self.world.readFile(parent_path+"/robots/kinova_with_robotiq_85.urdf")
        # robot
...
def solve_ik_kinova_gen3(goal_position, goal_quaternion=None, init_qpos=None):
    """
    Solve inverse kinematics using the klampt numerical solver
    
    Args
        goal_position: target positino of R3
        goal_orientation: target oreintation of R4
        init_qpos: initial joint angle

    """
    kinova_ik = KinovaGen3IK()
    if init_qpos is None:
        init_qpos = [0]*7
    kinova_ik.robot.setConfig(kinova_ik.joint_angles_to_config(init_qpos))
    if goal_quaternion is None:
        (res, angles) = kinova_ik.solve_ik(world_pos = goal_position)
    else:
        (res, angles) = kinova_ik.solve_ik(world_pos = goal_position, world_rot = goal_quaternion)

    return angles

if __name__ == "__main__":
    print(solve_ik_kinova_gen3([0.4, 0.2, 0.3]))